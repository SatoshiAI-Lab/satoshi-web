...
export const useChatMigrating = () => {
   ...
   * @returns
   */
  const messageHandler = (data: ChatResponse) => {
  ...
  const messageParser = async (response: ReadableStream<Uint8Array>) => {
    ...
    while (isReceiving) {
      ...
      utilParse.streamStrToJson(value, messageHandler)
    }
  }
  ...
  const handleLarge = async (
    ...
    utilParse.streamStrToJson(prevStr, messageHandler)
    resetSomeState()
  }
